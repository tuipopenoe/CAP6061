...
------------------------------------------------------------------------------  
    ...
    if (simGetScriptExecutionCount() == 0) then
        ...
        initial_angle, initial_distance = Vector_To_Goal()
    end
    ...
    Move_Towards_Goal = function()
        local dist, angle = Vector_To_Goal()
        limit = 5
    ...
    if state == 0 then
        ...
        if (obstacle_detected > 0 ) then
            ...
            mark_distance = Distance_To_Goal()
            -- Set the mark time
            mark_time = time.now
        else
            Move_Towards_Goal()
    ...
    elseif state == 1 then
        ...
        if minimum_distance <= current_distance then
        ...
        if (current_distance - mark_distance) < 0.1 and mark_time > 3 then
            mark = true
        end
        ...
        if mark == true and (current_distance - minimum_distance) < 0.1 then
            state = 0
            mark_time = 0
            mark_distance = 0
            Move_Towards_Goal()
